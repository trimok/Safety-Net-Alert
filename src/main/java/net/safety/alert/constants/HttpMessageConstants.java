package net.safety.alert.constants;

public class HttpMessageConstants {
	public final static String CREATE_PERSON = "CREATE_PERSON";
	public final static String UPDATE_PERSON = "UPDATE_PERSON";
	public final static String PATCH_PERSON = "PATCH_PERSON";
	public final static String DELETE_PERSON = "DELETE_PERSON";

	public final static String PERSON_NOT_FOUND = "PERSON NOT FOUND";
	public final static String PERSON_NOT_VALID = "FIRSTNAME AND LASTNAME MUST BE NOT EMPTY";
	public final static String PERSON_ALREADY_CREATED = "PERSON OR MEDICAL RECORD ALREADY CREATED";

	public final static String CREATE_MEDICAL_RECORD = "CREATE_MEDICAL_RECORD";
	public final static String UPDATE_MEDICAL_RECORD = "UPDATE_MEDICAL_RECORD";
	public final static String PATCH_MEDICAL_RECORD = "PATCH_MEDICAL_RECORD";
	public final static String DELETE_MEDICAL_RECORD = "DELETE_MEDICAL_RECORD";

	public final static String MEDICAL_RECORD_NOT_FOUND = "MEDICAL_RECORD NOT FOUND";
	public final static String MEDICAL_RECORD_ALREADY_CREATED = "PERSON OR MEDICAL_RECORD ALREADY CREATED";
	public final static String MEDICAL_RECORD_NOT_VALID = "FIRSTNAME AND LASTNAME MUST BE NOT EMPTY";

	public final static String CREATE_MAPPING_ADDRESS_STATION = "CREATE_MAPPING_ADDRESS_STATION";
	public final static String DELETE_MAPPING_ADDRESS_STATION = "DELETE_MAPPING_ADDRESS_STATION";
	public final static String UPDATE_MAPPING_ADDRESS_STATION = "UPDATE_MAPPING_ADDRESS_STATION";

	public final static String MAPPING_ADDRESS_STATION_NOT_FOUND = "MAPPING_ADDRESS_STATION NOT FOUND";
	public final static String MAPPING_ADDRESS_STATION_ALREADY_CREATED = " ADRESS OR MAPPING_ADDRESS_STATION ALREADY CREATED";
	public final static String MAPPING_ADDRESS_STATION_NOT_VALID = "ADDRESS AND STATION MUST BE NOT EMPTY";
}
